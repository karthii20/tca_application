...
def plot_slippage_through_time(df):
    ...
    st.write('Slippage throughout the time')
    fig_slippage_time = px.line(df, x='trade_time', y='slippage', labels={'trade_time': 'Trade Time', 'slippage': 'Slippage'})
    st.plotly_chart(fig_slippage_time, key=f"unique_key_for_chart1_{np.random.randint(0, 100000)}")
...
def plot_against_trade_volume(df, metric):
    ...
    fig_slippage_trade_vol = px.scatter(df, x='executed_amount', y=metric, color='position_name',
                                        labels={'executed_amount': 'Executed Amount', metric: metric.capitalize()})
    st.plotly_chart(fig_slippage_trade_vol, key=f"unique_key_for_chart2_{metric}_{np.random.randint(0, 100000)}")
...
def plot_slippage_distribution(df):
    ...
    fig_sippage_dist.update_layout(xaxis_title='Slippage', yaxis_title='Density')
    st.plotly_chart(fig_sippage_dist, key=f"unique_key_for_chart3_{np.random.randint(0, 100000)}")

def plot_trade_and_market_averages(df):
    st.write('Trades and market averages')
    fig_vwaps = make_subplots()
    fig_vwaps.add_trace(go.Line(x=df['trade_time'], y=df['trades_vwap'], name='trades vwap'))
    fig_vwaps.add_trace(go.Line(x=df['trade_time'], y=df['market_vwap'], name='market vwap'))
    fig_vwaps.add_trace(go.Line(x=df['trade_time'], y=df['trades_twap'], name='trades twap'))
    fig_vwaps.update_layout(xaxis_title='Trade Time', yaxis_title='Price')
    st.plotly_chart(fig_vwaps, key=f"unique_key_for_chart4_{np.random.randint(0, 100000)}")
...
def build_prices_plot(df, trade_type, my_price):
    ...
    fig_vwaps = make_subplots()
    fig_vwaps.add_trace(go.Line(x=df['trade_time'], y=df[my_price], name=my_price))
    fig_vwaps.add_trace(go.Line(x=df['trade_time'], y=df['arrival_price'], name='arrival price'))
    fig_vwaps.add_trace(go.Line(x=df['trade_time'], y=df['market_mid'], name='market mid'))
    fig_vwaps.add_trace(go.Line(x=df['trade_time'], y=df[best], name='market best'))
    fig_vwaps.add_trace(go.Line(x=df['trade_time'], y=df[worst], name='market worst'))
    fig_vwaps.add_trace(go.Line(x=df['trade_time'], y=df['market_vwap'], name='market vwap'))
    fig_vwaps.update_layout(xaxis_title='Trade Time', yaxis_title='Price')
    st.plotly_chart(fig_vwaps, key=f"unique_key_for_chart_{trade_type}_{my_price}_{np.random.randint(0, 100000)}")
...
def plot_slippage_by(df, metric):
    if metric == 'RIC':
        st.write(f'Average slippage by symbol')
    else:
    ...
    if metric == 'RIC':
        fig_slippage_venue.update_layout(xaxis_title='Symbol', yaxis_title='Slippage', yaxis2_title='Market Spread')
    else:
        fig_slippage_venue.update_layout(xaxis_title=metric.capitalize(), yaxis_title='Slippage', yaxis2_title='Market Spread')
    if metric == 'weekday':
    ...
    st.plotly_chart(fig_slippage_venue, key=f"unique_key_for_chart_{metric}_{np.random.randint(0, 100000)}")

def plot_slippage_dist_by(df, metric):
    df = df.dropna(subset='order_id')
    if metric == 'RIC':
        st.write(f'Slippage distribution by symbol')
    else:
    ...
    fig_sippage_dist = ff.create_distplot(hists, groups, show_hist=False)
    if metric == 'RIC':
        fig_sippage_dist.update_layout(
            xaxis_title='Symbol',
            yaxis_title='Density'
        )
    else:
    ...
    st.plotly_chart(fig_sippage_dist, key=f"unique_key_for_chart_{metric}_dist_{np.random.randint(0, 100000)}")
...
def plot_trade_recap(df, order_id):
    ...
    if order_id:
        ...
        st.plotly_chart(fig5, key=f"unique_key_for_chart8_{np.random.randint(0, 100000)}")
...
def plot_arrival_latemcy(df):
    ...
    fig_arrival_latency.update_traces(name="Arrival Latency", showlegend=True)
    st.plotly_chart(fig_arrival_latency, key=f"unique_key_for_chart9_{np.random.randint(0, 100000)}")
